...
import homePOI from "../utils/homePOI.js"
...
class ROSController {
    ...
    async streamAudioFile(filePath, chunkSize = 1024 * 512) {
        // 512KB chunks
    ...
    async sendGoToCommand(POI) {
        const move_base_publisher = new ROS.Topic({
            ros: this.ros,
        ...
        try {
            ...
            console.error("Failed to go to POI:", error);
        }
    }
        ...
        switch (command) {
            case "forward":
                message = new ROS.Message({
                    linear: {
                        x: parseFloat(process.env.FORWARD_SPEED),
                        y: 0,
                        z: 0
                    },
                    angular: {
            ...
            case "left":
                message = new ROS.Message({
                    ...
                    angular: {
                        x: 0,
                        y: 0,
                        z: parseFloat(process.env.TURN_SPEED)
                    }
                });
                break;
            case "right":
